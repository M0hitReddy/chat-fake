  ...
  useEffect(() => {
    ...
    state.socket.on('receive-message', (data) => {
      ...
      if (data.sender !== data.receiver) {
        console.log(data.sender + state.selectedChat)
            ...
            <div className="flex flex-col flex-grow overflow-y-auto justify-end gap-3  whitespace-pre-wrap p-4 text-sm">
              {/* <div className="flex-grow"/> */}
              {
                state.messages.map((message) => (
                  <div key={message.time} className={`${(user.id === message.senderId) ? 'self-end bg-primary rounded-tr-none' : 'self-start shadow-lg bg-secondary rounded-tl-none'} relative pe-12 max-w-full sm:max-w-[90%] md:max-w-[70%] break-words flex gap-2 w-auto px-2 py-1 rounded-xl `}>
                    {/* <div className="flex"> */}
                    <p className={`text-primary${(user.id === message.senderId) ? '-foreground' : '-background'} text-md font-medium leading-7 h-max `}>{message.message}</p>
                    {/* <p className="pe-14 h-1"></p> */}
                    {/* </div> */}
                    <p className="absolute bottom-1 right-2 w-max  text-gray-400 text-[10px] leading-3 self-end ">{getTime(message.time)}</p>
                  </div>
                ))
              }
